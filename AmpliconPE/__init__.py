...
class MasterRead(str):
    def score(self, FWD_read, REV_read):
        return self.sw.score(self.seq, FWD_read) + self.sw.score(
            self.reverse_compliment, REV_read
        )
    ...
    def extract_barcode(self, FWD_read, REV_read):
        fwd_bc = self.sw.extract_barcode(
            FWD_read, self.seq, self.barcode_start, self.barcode_stop
        )
        rev_bc = reverse_compliment(
            self.sw.extract_barcode(
                REV_read, self.reverse_compliment, self.rc_start, self.rc_stop
            )
        )
